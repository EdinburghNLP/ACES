""" Adapted from https://github.com/Unbabel/COMET/blob/explainable-metrics/explainable-metrics/explain_comet.py
Script to run the explanations on COMET and to save alignment scores for later use

python explain_comet.py -m PATH/TO/COMET.ckpt -t PATH/TO/DATA -o PATH/TO/OUTPUT/DIR --batch_size 8
"""
...
if __name__ == "__main__":
    ...
    for testset in args.testset[0]:    
        data = pd.read_csv(testset)   # for WMT data (from their repo)
        # data = pd.read_csv(testset, quoting=csv.QUOTE_NONE, escapechar="\\")  # for reading merged.tsv (ACES)
        ...
        #  ------------------------- Save SRC Alignment Explanations --------------------
        scores = []
        input_ids = []
        in_span_mask = []
        for batch in align_subword_scores:
        ...
        with open(os.path.join(out_path, 'src_scores.json'), "w") as f:
        ...
        #  ------------------------- Save REF Alignment Explanations --------------------
        scores = []
        input_ids = []
        in_span_mask = []
        for batch in align_subword_scores:
            ...
            input_ids.extend(batch['input_ids'].numpy(force=True).tolist())
            in_span_mask.extend(batch['in_span_mask'].numpy(force=True).tolist())
        with open(os.path.join(out_path, 'ref_scores.json'), "w") as f:
            json.dump({'scores':scores, 'input_ids':input_ids, 'in_span_mask':in_span_mask}, f)
        
        #  ------------------------- Uni (SRC+REF) Alignment Explanations --------------------
        align_subword_scores = []
        for batch in tqdm(model_inputs):
        ...
        #  ------------------------- Save Uni (SRC+REF) Alignment Explanations --------------------
        scores = []
        input_ids = []
        in_span_mask = []
        for batch in align_subword_scores:
            ...
            input_ids.extend(batch['input_ids'].numpy(force=True).tolist())
            in_span_mask.extend(batch['in_span_mask'].numpy(force=True).tolist())
        with open(os.path.join(out_path, 'src_ref_scores.json'), "w") as f:
            json.dump({'scores':scores, 'input_ids':input_ids, 'in_span_mask':in_span_mask}, f)
